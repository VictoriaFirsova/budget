...
class UploadPaymentFileView(FormView):
    ...
    def post(self, request, **kwargs):
        try:
            ...
            if not mime_type or mime_type not in (
                ...
                messages.error(request, 'Недопустимый формат файла. Пожалуйста, загрузите файл формата CSV или XLSX.')
                return render(request, self.template_name, {'form': self.form_class()})
            ...
            card_type = self.determine_card_type(uploaded_file)
            uploaded_file.seek(0)
            self.save_statement(self.change_rate(self.read_csv_content(csv_content, detected_encoding, card_type)))
    ...
    def read_csv_content(self, csv_content, detected_encoding, card_type):
        if card_type == 'Priorbank':
            ...
            io_data = io.BytesIO(csv_content.encode(detected_encoding))
            data = pd.read_csv(io_data, delimiter=',', skiprows=1, parse_dates=True, )  # убрать после отладки nrows=5,
            ...
            for i in df.iterrows():
                ...
                try:
                    ...
                    if df.loc[i[0], 'category'] == 'Другое' or df.loc[i[0], 'category'] == 'nan':
                        try:
    ...
    def get_category(self, category_name):
        ...
        categories_mapping = {
            'Бензин': ['АЗС', 'Транспортировка', 'Бизнес услуги',
                       'Оптовые поставщики и производители', 'Parking Lots and Garages'],
            'Здоровье': ['Медицинский сервис', 'Аптеки', 'Hospitals',
                         'Medical Services Health Practitioners - No Elsewhere Classified'],
            'Машина': ['Автомобили - продажа / сервис', 'Direct Marketing Insurance Services'],
            'Продукты': ['Магазины продуктовые', 'Grocery Stores and Supermarkets',
            ...
            'Жилье': ['Аренда жилья / отели и мотели', 'Lodging - Hotels, Motels, and Resorts'],
            'Развлечения': ['Развлечения', 'Amusement Parks, Circuses, Carnivals, and Fortune Tellers',
        ...
        mapped_category_name = 'Другое'  # Значение по умолчанию, если категория не найдена
